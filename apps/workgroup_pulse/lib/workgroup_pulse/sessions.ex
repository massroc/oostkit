defmodule WorkgroupPulse.Sessions do
  ...
  def advance_question(%Session{state: "scoring"} = session) do
    ...
    WorkgroupPulse.Scoring.lock_row(session, session.current_question_index)

    next_question_index = session.current_question_index + 1

    # Check if we're revisiting a question that already has scores
    is_revisit = WorkgroupPulse.Scoring.has_scores?(session, next_question_index)
    ...
    turn_index =
      if is_revisit do
        ...
        locked_count =
          WorkgroupPulse.Scoring.count_locked_turns(session, next_question_index)
  ...
  def get_skipped_participants(%Session{} = session, question_index) do
    participants = get_participants_in_turn_order(session)
    scores = WorkgroupPulse.Scoring.list_scores_for_question(session, question_index)
