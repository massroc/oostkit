    ...
    <% else %>
      <%= if @carousel_index in 4..6 do %>
        ...
        >
          <.sheet
            variant={:secondary}
            class="notes-panel p-4 w-full h-full shadow-sheet overflow-hidden whitespace-nowrap"
              ...
              <div class="space-y-1.5">
                <%= if length(@question_notes) > 0 do %>
                  <%= for note <- @question_notes do %>
                    <div class="bg-surface-sheet/50 rounded p-2 text-sm">
                      <p class="font-workshop text-ink-blue">{note.content}</p>
                    </div>
                  <% end %>
                <% else %>
