defmodule WorkgroupPulse.FacilitationTest do
  ...
  describe "segment-based timer functions" do
    ...
    test "calculate_segment_duration/1 returns nil for nil duration", %{session: session} do
      # Session created without planned_duration_minutes should return nil
      assert session.planned_duration_minutes == nil
    ...
    test "current_timer_phase/1 returns summary_actions for actions state" do
      session = %WorkgroupPulse.Sessions.Session{
        state: "actions",
        current_question_index: 7
      }
    ...
    test "phase_name/1 returns correct name for summary_actions" do
