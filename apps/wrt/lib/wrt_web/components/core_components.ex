defmodule WrtWeb.CoreComponents do
  ...
  This module provides core building blocks for the application,
  such as buttons, forms, and flash messages.
  """
  ...
  def input(%{type: "select"} = assigns) do
    ...
    <div>
      ...
      <select
        id={@id}
        name={@name}
        class="block w-full rounded-md border border-gray-300 bg-white shadow-sm focus:border-ok-purple-400 focus:ring-0 sm:text-sm"
        multiple={@multiple}
        {@rest}
      >
  ...
  def label(assigns) do
    ...
    <label
      for={@for}
      class="block text-xs font-semibold text-zinc-600 mb-1.5 font-brand uppercase tracking-wide"
    >
      {render_slot(@inner_block)}
    </label>
    """
