defmodule Portal.Accounts do
  ...
  This bypasses email confirmation and creates an enabled super admin account.
  """
  def create_super_admin(attrs) do
    %User{role: "super_admin", enabled: true, confirmed_at: DateTime.utc_now(:second)}
  ...
  @doc """
  Creates a session manager user (admin only).

  The user will need to set their password via the magic link login flow.
  """
  def create_session_manager(attrs) do
    %User{role: "session_manager", enabled: true}
    |> User.email_changeset(attrs)
    |> maybe_add_name(attrs)
    |> Repo.insert()
  end

  defp maybe_add_name(changeset, attrs) do
    case Map.get(attrs, :name) || Map.get(attrs, "name") do
      nil -> changeset
      name -> Ecto.Changeset.put_change(changeset, :name, name)
    end
  end
  ...
  @doc """
  Updates a user's profile (name, role, enabled status).
  """
  def update_user(%User{} = user, attrs) do
    user
    |> Ecto.Changeset.change(attrs)
    |> Ecto.Changeset.validate_inclusion(:role, User.roles())
    |> Repo.update()
  end
  ...
  See `Portal.Accounts.User.email_changeset/3` for a list of supported options.
  ...
  See `Portal.Accounts.User.password_changeset/3` for a list of supported options.
