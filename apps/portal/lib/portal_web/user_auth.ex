defmodule PortalWeb.UserAuth do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      PortalWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule PortalWeb.PageLive do
        use PortalWeb, :live_view

        on_mount {PortalWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{PortalWeb.UserAuth, :require_authenticated}] do
  ...
  def on_mount(:require_super_admin, _params, _session, socket) do
    ...
    if user && Accounts.User.super_admin?(user) && Accounts.User.enabled?(user) do
      {:cont, socket}
    else
      socket =
        socket
        |> Phoenix.LiveView.put_flash(:error, "You must be a super admin to access this page.")
