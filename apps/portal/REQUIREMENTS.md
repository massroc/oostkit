...
- **Settings** (`/users/settings`) -- compact single-column layout (`max-w-xl`) with separate cards per section. Each card is a self-contained form with `rounded-xl bg-surface-sheet p-4 shadow-sheet` styling. Uses compact 6-column field grids (`grid-cols-6 gap-x-3 gap-y-2`) with Petal's `no_margin` attribute to eliminate default field spacing. Forms are sibling elements (no nesting) with each card containing its own `<.form>`. Sections: General Information (name + org side-by-side on desktop), Email & Preferences (email change + product_updates checkbox with auto-save on change), Password (add/change), and Danger Zone (inline delete with `ring-ok-red-200` warning ring). Product updates checkbox defaults to true for new users and auto-saves on change without a submit button. Referral source is collected at registration only and not editable in settings. The settings page loads without requiring sudo mode; sudo checks are performed in handlers for sensitive actions (email change, password change, account deletion) with a graceful redirect to login if not in sudo mode.
